...
"]
    ...
    loop {
        ...
        for i in [1, 0, 1].iter().cloned() { // [0, 1, 0, 1] will prevent issue
            println!("i: {}", i);
            let vector: Vec<Vertex> = vec!(
                Vertex { position: [-0.5, -0.25] },
                Vertex { position: [0.0, 0.5] },
                Vertex { position: [0.25, -0.1] }
            ).iter().cycle().take(i * 3).cloned().collect();
            let buffer = pool.chunk(vector);
            //if i != 0 { // uncommenting this will prevent the issue.
